...
fn seeded_range_store() -> MemoryStore {
    let store = mem_store();
    let txn = store.begin(false).unwrap();
    let cf = txn.cf(CF).unwrap();
...
#[test]
fn scan_range_reverse_unbounded() {
    // .. reversed → every key, descending (including the out-of-`k:` sentinels).
    let store = seeded_range_store();
    let txn = store.begin(true).unwrap();
    let cf = txn.cf(CF).unwrap();
    let keys: Vec<_> = txn
...
fn scan_range_covers_no_keys() {
    ...
    // Range past all keys.
    let entries: Vec<_> = txn
        .scan_range(&cf, b"k:x".to_vec()..b"k:z".to_vec(), false)
        .unwrap()
        .map(|r| r.unwrap())
        .collect();
    assert!(entries.is_empty());
