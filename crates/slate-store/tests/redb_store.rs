...
fn seeded_range_store() -> (RedbStore, tempfile::TempDir) {
...
fn scan_range_reverse_descending() {
    // [k:b, k:e) reversed → k:d, k:c, k:b (same set, descending).
    let (store, _dir) = seeded_range_store();
    let txn = store.begin(true).unwrap();
    let cf = txn.cf(CF).unwrap();
    let keys: Vec<_> = txn
...
#[test]
fn scan_range_write_path_sees_own_writes() {
    // Exercise the eager write-path collection (collect_range_write): scan a
    // range over uncommitted writes within an open write transaction.
    let (store, _dir) = temp_store();
    let txn = store.begin(false).unwrap();
    let cf = txn.cf(CF).unwrap();
    txn.put(&cf, b"k:a", b"1").unwrap();
    ...
    // [k:b, k:d) → k:b, k:c (k:d excluded), ascending.
    let keys: Vec<_> = txn
        .scan_range(&cf, b"k:b".to_vec()..b"k:d".to_vec(), false)
        .unwrap()
        .map(|r| r.unwrap().0)
    ...
    // Same range, reverse.
    let keys_rev: Vec<_> = txn
        .scan_range(&cf, b"k:b".to_vec()..b"k:d".to_vec(), true)
        .unwrap()
        .map(|r| r.unwrap().0)
        .collect();
    assert_eq!(keys_rev, vec![b"k:c".to_vec(), b"k:b".to_vec()]);

    // Inclusive end on the write path: [k:b, k:d] → k:b, k:c, k:d.
    let keys_incl: Vec<_> = txn
        .scan_range(&cf, b"k:b".to_vec()..=b"k:d".to_vec(), false)
        .unwrap()
        .map(|r| r.unwrap().0)
        .collect();
