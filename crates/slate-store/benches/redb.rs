...
fn new_store(dir: &std::path::Path) -> RedbStore {
    let store = RedbStore::open(&dir.join("bench.redb")).unwrap();
    store.create_cf(CF).unwrap();
    store
}

fn seeded_store(dir: &std::path::Path, n: usize) -> RedbStore {
...
fn bench_put(c: &mut Criterion) {
    let mut group = c.benchmark_group("redb/put");
    for n in [100, 1_000] {
...
fn bench_put_batch(c: &mut Criterion) {
    let mut group = c.benchmark_group("redb/put_batch");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
        let store = new_store(dir.path());
        let pairs = generate_kv_pairs(n);
...
fn bench_get(c: &mut Criterion) {
    let mut group = c.benchmark_group("redb/get");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
        let store = seeded_store(dir.path(), n);
        let pairs = generate_kv_pairs(n);
...
fn bench_multi_get(c: &mut Criterion) {
    let mut group = c.benchmark_group("redb/multi_get");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
        let store = seeded_store(dir.path(), n);
        let pairs = generate_kv_pairs(n);
...
fn bench_scan_prefix(c: &mut Criterion) {
    let mut group = c.benchmark_group("redb/scan_prefix");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
        let store = seeded_store(dir.path(), n);
...
fn bench_delete(c: &mut Criterion) {
    let mut group = c.benchmark_group("redb/delete");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
        let store = seeded_store(dir.path(), n);
        let pairs = generate_kv_pairs(n);
...
fn bench_delete_batch(c: &mut Criterion) {
    let mut group = c.benchmark_group("redb/delete_batch");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
        let store = seeded_store(dir.path(), n);
        let pairs = generate_kv_pairs(n);
