...
fn new_store(dir: &std::path::Path) -> RocksStore {
    let store = RocksStore::open(dir).unwrap();
    store.create_cf(CF).unwrap();
    store
}

fn seeded_store(dir: &std::path::Path, n: usize) -> RocksStore {
...
fn bench_put(c: &mut Criterion) {
    let mut group = c.benchmark_group("rocks/put");
    for n in [100, 1_000] {
...
fn bench_put_batch(c: &mut Criterion) {
    let mut group = c.benchmark_group("rocks/put_batch");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
        let store = new_store(dir.path());
        let pairs = generate_kv_pairs(n);
...
fn bench_get(c: &mut Criterion) {
    let mut group = c.benchmark_group("rocks/get");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
...
fn bench_multi_get(c: &mut Criterion) {
    let mut group = c.benchmark_group("rocks/multi_get");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
        let store = seeded_store(dir.path(), n);
        let pairs = generate_kv_pairs(n);
...
fn bench_scan_prefix(c: &mut Criterion) {
    let mut group = c.benchmark_group("rocks/scan_prefix");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
        let store = seeded_store(dir.path(), n);
...
fn bench_delete(c: &mut Criterion) {
    let mut group = c.benchmark_group("rocks/delete");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
        let store = seeded_store(dir.path(), n);
        let pairs = generate_kv_pairs(n);
...
fn bench_delete_batch(c: &mut Criterion) {
    let mut group = c.benchmark_group("rocks/delete_batch");
    for n in [100, 1_000] {
        let dir = tempfile::tempdir().unwrap();
        let store = seeded_store(dir.path(), n);
        let pairs = generate_kv_pairs(n);
