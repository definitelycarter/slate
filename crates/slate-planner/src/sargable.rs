...
//!
//! This module also recognizes the Mongo implicit-equality idioms the find
//! front-end emits (`field = v OR ARRAY_CONTAINS(field, v)`, `MultikeyEq`), so
...
pub(crate) fn plan_source(
    ...
    // The conjunct is NOT consumed: it stays as a residual recheck. A numeric
    // index scan is already exact (`slate-executor`'s `index_scan` does a full
    // scan + coercing `compare_bson` post-filter), so for those the recheck is
    // redundant; it's kept for uniformity with the top-level-OR path and as a
    // safety net for typed (string/date) range scans, whose exactness rests on
    // the index key order matching the evaluator's comparison. The index narrows
    // the candidate set; the recheck keeps the result precise.
    for (i, conjunct) in conjuncts.iter().enumerate() {
        if consumed.contains(&i) {
            continue;
        }
