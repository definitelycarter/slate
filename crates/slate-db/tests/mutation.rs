mod common;
use common::*;

use bson::{Bson, doc, rawdoc};
use slate_db::v2::{IndexOptions, TriggerFunction};
use slate_db::{DatabaseBuilder, TriggerCtx, TriggerError};
use slate_store::MemoryStore;

// ── Mutation operator tests ─────────────────────────────────────

fn get_str_array(doc: &bson::RawDocumentBuf, path: &str) -> Vec<String> {
    let parsed: bson::Document = bson::deserialize_from_slice(doc.as_bytes()).unwrap();
    let segments: Vec<&str> = path.split('.').collect();
    let mut current = &parsed;
    for seg in &segments[..segments.len() - 1] {
        current = current.get_document(seg).unwrap();
    }
    current
        .get_array(segments.last().unwrap())
        .unwrap()
        .iter()
        .map(|v| v.as_str().unwrap().to_string())
        .collect()
}

#[test]
fn mutation_set_explicit() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice", "status": "active" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$set": { "status": "archived", "score": 100 } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(d.get_str("name").unwrap(), "Alice");
    assert_eq!(d.get_str("status").unwrap(), "archived");
    assert_eq!(d.get_i32("score").unwrap(), 100);
}

#[test]
fn mutation_unset() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice", "status": "active", "score": 50 })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$unset": { "score": "" } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(d.get_str("name").unwrap(), "Alice");
    assert_eq!(d.get_str("status").unwrap(), "active");
    assert!(!d.get_check("score"));
}

#[test]
fn mutation_inc_i32() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "score": 10_i32 })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$inc": { "score": 5_i32 } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(d.get_i32("score").unwrap(), 15);
}

#[test]
fn mutation_inc_missing_field() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$inc": { "score": 7_i32 } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(d.get_i32("score").unwrap(), 7);
}

#[test]
fn mutation_inc_negative_decrement() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "score": 100_i32 })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$inc": { "score": -30_i32 } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(d.get_i32("score").unwrap(), 70);
}

#[test]
fn mutation_inc_f64() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "balance": 100.50_f64 })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$inc": { "balance": 25.25_f64 } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert!((d.get_f64("balance").unwrap() - 125.75).abs() < f64::EPSILON);
}

#[test]
fn mutation_rename() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "old_name": "Alice", "status": "active" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$rename": { "old_name": "name" } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(d.get_str("name").unwrap(), "Alice");
    assert!(!d.get_check("old_name"));
    assert_eq!(d.get_str("status").unwrap(), "active");
}

#[test]
fn mutation_push() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "tags": ["rust", "db"] })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$push": { "tags": "perf" } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(get_str_array(&d, "tags"), vec!["rust", "db", "perf"]);
}

#[test]
fn mutation_push_creates_array() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$push": { "tags": "new" } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(get_str_array(&d, "tags"), vec!["new"]);
}

#[test]
fn mutation_lpush() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "queue": ["second", "third"] })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$lpush": { "queue": "first" } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(get_str_array(&d, "queue"), vec!["first", "second", "third"]);
}

#[test]
fn mutation_pop() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "stack": ["a", "b", "c"] })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$pop": { "stack": 1 } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(get_str_array(&d, "stack"), vec!["a", "b"]);
}

#[test]
fn mutation_multiple_operators() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice", "score": 10_i32, "tags": ["a"] })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! {
            "$set": { "name": "Alice Updated" },
            "$inc": { "score": 5_i32 },
            "$push": { "tags": "b" },
        })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(d.get_str("name").unwrap(), "Alice Updated");
    assert_eq!(d.get_i32("score").unwrap(), 15);
    assert_eq!(get_str_array(&d, "tags"), vec!["a", "b"]);
}

#[test]
fn mutation_bare_fields_implicit_set() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice", "status": "active" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "status": "archived", "score": 99 })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(d.get_str("name").unwrap(), "Alice");
    assert_eq!(d.get_str("status").unwrap(), "archived");
    assert_eq!(d.get_i32("score").unwrap(), 99);
}

#[test]
fn mutation_dot_path_set() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "address": { "city": "Austin", "state": "TX" } })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$set": { "address.city": "Denver" } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    let addr = d.get_document("address").unwrap();
    assert_eq!(addr.get_str("city").unwrap(), "Denver");
    assert_eq!(addr.get_str("state").unwrap(), "TX");
}

#[test]
fn mutation_dot_path_inc() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "stats": { "views": 100_i32, "likes": 10_i32 } })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$inc": { "stats.views": 1_i32 } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    let stats = d.get_document("stats").unwrap();
    assert_eq!(stats.get_i32("views").unwrap(), 101);
    assert_eq!(stats.get_i32("likes").unwrap(), 10);
}

#[test]
fn mutation_dot_path_creates_intermediates() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$set": { "address.city": "Austin" } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(d.get_str("name").unwrap(), "Alice");
    let addr = d.get_document("address").unwrap();
    assert_eq!(addr.get_str("city").unwrap(), "Austin");
}

#[test]
fn mutation_dot_path_unset() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(
            doc! { "_id": "r1", "address": { "city": "Austin", "state": "TX", "zip": "78701" } },
        )
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$unset": { "address.zip": "" } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    let addr = d.get_document("address").unwrap();
    assert_eq!(addr.get_str("city").unwrap(), "Austin");
    assert_eq!(addr.get_str("state").unwrap(), "TX");
    assert!(!addr.get_check("zip"));
}

#[test]
fn mutation_dot_path_push() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "data": { "items": ["a"] } })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$push": { "data.items": "b" } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(get_str_array(&d, "data.items"), vec!["a", "b"]);
}

#[test]
fn mutation_update_many_with_inc() {
    let (db, _dir) = temp_db();
    seed_records(&db);

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("status", Bson::String("active".into()));
    let result = db
        .collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$inc": { "revenue": 1000.0_f64 } })
        .execute(&txn)
        .unwrap()
        .affected;
    assert_eq!(result, 3);
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "acct-1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert!((d.get_f64("revenue").unwrap() - 51000.0).abs() < f64::EPSILON);

    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "acct-4" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert!((d.get_f64("revenue").unwrap() - 96000.0).abs() < f64::EPSILON);

    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "acct-5" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert!((d.get_f64("revenue").unwrap() - 201000.0).abs() < f64::EPSILON);

    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "acct-2" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert!((d.get_f64("revenue").unwrap() - 80000.0).abs() < f64::EPSILON);
}

#[test]
fn mutation_index_maintained_on_set() {
    let (db, _dir) = temp_db();
    let txn = db.begin(false).unwrap();
    db.collections().create("idx_mut").execute(&txn).unwrap();
    db.collection("idx_mut")
        .indexes()
        .create("status", IndexOptions::default())
        .execute(&txn)
        .unwrap();
    db.collection("idx_mut")
        .insert_one(doc! { "_id": "r1", "name": "Alice", "status": "active" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection("idx_mut")
        .find(&filter)
        .update(doc! { "$set": { "status": "archived" } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let results = db
        .collection("idx_mut")
        .find(eq_filter("status", Bson::String("active".into())))
        .iter_raw(&txn)
        .unwrap()
        .collect::<Result<Vec<_>, _>>()
        .unwrap();
    assert_eq!(results.len(), 0);

    let results = db
        .collection("idx_mut")
        .find(eq_filter("status", Bson::String("archived".into())))
        .iter_raw(&txn)
        .unwrap()
        .collect::<Result<Vec<_>, _>>()
        .unwrap();
    assert_eq!(results.len(), 1);
    assert_eq!(results[0].get_str("_id").unwrap(), "r1");
}

#[test]
fn mutation_index_maintained_on_unset() {
    let (db, _dir) = temp_db();
    let txn = db.begin(false).unwrap();
    db.collections().create("idx_unset").execute(&txn).unwrap();
    db.collection("idx_unset")
        .indexes()
        .create("status", IndexOptions::default())
        .execute(&txn)
        .unwrap();
    db.collection("idx_unset")
        .insert_one(doc! { "_id": "r1", "name": "Alice", "status": "active" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection("idx_unset")
        .find(&filter)
        .update(doc! { "$unset": { "status": "" } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let results = db
        .collection("idx_unset")
        .find(eq_filter("status", Bson::String("active".into())))
        .iter_raw(&txn)
        .unwrap()
        .collect::<Result<Vec<_>, _>>()
        .unwrap();
    assert_eq!(results.len(), 0);

    let d = db
        .collection("idx_unset")
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(d.get_str("name").unwrap(), "Alice");
    assert!(!d.get_check("status"));
}

#[test]
fn mutation_push_pop_as_stack() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "stack" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    for val in ["a", "b", "c"] {
        let txn = db.begin(false).unwrap();
        let filter = eq_filter("_id", Bson::String("r1".into()));
        db.collection(COLLECTION)
            .find(&filter)
            .update(doc! { "$push": { "items": val } })
            .one()
            .execute(&txn)
            .unwrap();
        txn.commit().unwrap();
    }

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(get_str_array(&d, "items"), vec!["a", "b", "c"]);

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$pop": { "items": 1 } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(get_str_array(&d, "items"), vec!["a", "b"]);
}

#[test]
fn mutation_lpush_pop_as_queue() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "queue" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    for val in ["first", "second", "third"] {
        let txn = db.begin(false).unwrap();
        let filter = eq_filter("_id", Bson::String("r1".into()));
        db.collection(COLLECTION)
            .find(&filter)
            .update(doc! { "$lpush": { "items": val } })
            .one()
            .execute(&txn)
            .unwrap();
        txn.commit().unwrap();
    }

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(get_str_array(&d, "items"), vec!["third", "second", "first"]);

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$pop": { "items": 1 } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let d = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(get_str_array(&d, "items"), vec!["third", "second"]);
}

#[test]
fn mutation_unknown_operator_rejected() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    let result = db
        .collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$badop": { "name": "Bob" } })
        .one()
        .execute(&txn);
    let err = match result {
        Err(e) => e.to_string(),
        Ok(_) => panic!("expected error for $badop"),
    };
    assert!(
        err.contains("$badop"),
        "error should mention the bad op: {err}"
    );
}

#[test]
fn mutation_id_rejected() {
    let (db, _dir) = temp_db();
    create_collection(&db, COLLECTION);

    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    let result = db
        .collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$set": { "_id": "r2" } })
        .one()
        .execute(&txn);
    assert!(result.is_err());
}

// ── Delete trigger tests ──────────────────────────────────────────

#[test]
fn delete_fires_trigger_successfully() {
    let db = DatabaseBuilder::new()
        .with_trigger("noop", |_: &TriggerCtx<'_>| Ok(()))
        .with_trigger("boom", |_: &TriggerCtx<'_>| {
            Err(TriggerError::Body("trigger failed!".into()))
        })
        .open(MemoryStore::new())
        .unwrap();

    let txn = db.begin(false).unwrap();
    db.collections().create(COLLECTION).execute(&txn).unwrap();
    db.collection(COLLECTION)
        .triggers()
        .create("audit", TriggerFunction::from_name("noop"))
        .execute(&txn)
        .unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice" })
        .execute(&txn)
        .unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r2", "name": "Bob" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    // Delete with trigger registered — should succeed
    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    let count = db
        .collection(COLLECTION)
        .find(&filter)
        .delete()
        .one()
        .execute(&txn)
        .unwrap()
        .affected;
    assert_eq!(count, 1);
    txn.commit().unwrap();

    // Verify the record is actually gone
    let txn = db.begin(true).unwrap();
    let result = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap();
    assert!(result.is_none());
    let result = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r2" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap();
    assert!(result.is_some());
}

#[test]
fn delete_many_fires_trigger_successfully() {
    let db = DatabaseBuilder::new()
        .with_trigger("noop", |_: &TriggerCtx<'_>| Ok(()))
        .with_trigger("boom", |_: &TriggerCtx<'_>| {
            Err(TriggerError::Body("trigger failed!".into()))
        })
        .open(MemoryStore::new())
        .unwrap();

    let txn = db.begin(false).unwrap();
    db.collections().create(COLLECTION).execute(&txn).unwrap();
    db.collection(COLLECTION)
        .triggers()
        .create("log_delete", TriggerFunction::from_name("noop"))
        .execute(&txn)
        .unwrap();
    for i in 1..=5 {
        db.collection(COLLECTION)
            .insert_one(
                doc! { "_id": format!("r{i}"), "status": if i <= 3 { "active" } else { "inactive" } },
            )
            .execute(&txn)
            .unwrap();
    }
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("status", Bson::String("active".into()));
    let count = db
        .collection(COLLECTION)
        .find(&filter)
        .delete()
        .execute(&txn)
        .unwrap()
        .affected;
    assert_eq!(count, 3);
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let remaining = db
        .collection(COLLECTION)
        .find(rawdoc! {})
        .iter_raw(&txn)
        .unwrap()
        .collect::<Result<Vec<_>, _>>()
        .unwrap();
    assert_eq!(remaining.len(), 2);
}

#[test]
fn delete_trigger_error_propagates() {
    let db = DatabaseBuilder::new()
        .with_trigger("noop", |_: &TriggerCtx<'_>| Ok(()))
        .with_trigger("boom", |_: &TriggerCtx<'_>| {
            Err(TriggerError::Body("trigger failed!".into()))
        })
        .open(MemoryStore::new())
        .unwrap();

    // Seed data first (no triggers yet)
    let txn = db.begin(false).unwrap();
    db.collections().create(COLLECTION).execute(&txn).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    // Register bad trigger in a separate transaction
    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .triggers()
        .create("bad_trigger", TriggerFunction::from_name("boom"))
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    // Delete should fail because the trigger errors
    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    let result = db
        .collection(COLLECTION)
        .find(&filter)
        .delete()
        .one()
        .execute(&txn);
    assert!(result.is_err());
}

// ── Insert trigger tests ─────────────────────────────────────────

#[test]
fn insert_fires_trigger_successfully() {
    let db = DatabaseBuilder::new()
        .with_trigger("noop", |_: &TriggerCtx<'_>| Ok(()))
        .with_trigger("boom", |_: &TriggerCtx<'_>| {
            Err(TriggerError::Body("trigger failed!".into()))
        })
        .open(MemoryStore::new())
        .unwrap();

    let txn = db.begin(false).unwrap();
    db.collections().create(COLLECTION).execute(&txn).unwrap();
    db.collection(COLLECTION)
        .triggers()
        .create("audit", TriggerFunction::from_name("noop"))
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    // Insert with trigger registered — should succeed
    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let result = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap();
    assert!(result.is_some());
    assert_eq!(result.unwrap().get_str("name").unwrap(), "Alice");
}

#[test]
fn insert_trigger_error_propagates() {
    let db = DatabaseBuilder::new()
        .with_trigger("noop", |_: &TriggerCtx<'_>| Ok(()))
        .with_trigger("boom", |_: &TriggerCtx<'_>| {
            Err(TriggerError::Body("trigger failed!".into()))
        })
        .open(MemoryStore::new())
        .unwrap();

    let txn = db.begin(false).unwrap();
    db.collections().create(COLLECTION).execute(&txn).unwrap();
    db.collection(COLLECTION)
        .triggers()
        .create("bad_trigger", TriggerFunction::from_name("boom"))
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let result = db
        .collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice" })
        .execute(&txn);
    assert!(result.is_err());
}

// ── Update trigger tests ─────────────────────────────────────────

#[test]
fn update_fires_trigger_successfully() {
    let db = DatabaseBuilder::new()
        .with_trigger("noop", |_: &TriggerCtx<'_>| Ok(()))
        .with_trigger("boom", |_: &TriggerCtx<'_>| {
            Err(TriggerError::Body("trigger failed!".into()))
        })
        .open(MemoryStore::new())
        .unwrap();

    let txn = db.begin(false).unwrap();
    db.collections().create(COLLECTION).execute(&txn).unwrap();
    db.collection(COLLECTION)
        .triggers()
        .create("audit", TriggerFunction::from_name("noop"))
        .execute(&txn)
        .unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice", "score": 10 })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$set": { "score": 99 } })
        .one()
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let doc = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(doc.get_i32("score").unwrap(), 99);
}

#[test]
fn update_trigger_error_propagates() {
    let db = DatabaseBuilder::new()
        .with_trigger("noop", |_: &TriggerCtx<'_>| Ok(()))
        .with_trigger("boom", |_: &TriggerCtx<'_>| {
            Err(TriggerError::Body("trigger failed!".into()))
        })
        .open(MemoryStore::new())
        .unwrap();

    // Seed data first (no triggers yet)
    let txn = db.begin(false).unwrap();
    db.collections().create(COLLECTION).execute(&txn).unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    // Register bad trigger in a separate transaction
    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .triggers()
        .create("bad_trigger", TriggerFunction::from_name("boom"))
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    let result = db
        .collection(COLLECTION)
        .find(&filter)
        .update(doc! { "$set": { "name": "Bob" } })
        .one()
        .execute(&txn);
    assert!(result.is_err());
}

// ── Replace trigger tests ────────────────────────────────────────

#[test]
fn replace_fires_trigger_successfully() {
    let db = DatabaseBuilder::new()
        .with_trigger("noop", |_: &TriggerCtx<'_>| Ok(()))
        .with_trigger("boom", |_: &TriggerCtx<'_>| {
            Err(TriggerError::Body("trigger failed!".into()))
        })
        .open(MemoryStore::new())
        .unwrap();

    let txn = db.begin(false).unwrap();
    db.collections().create(COLLECTION).execute(&txn).unwrap();
    db.collection(COLLECTION)
        .triggers()
        .create("audit", TriggerFunction::from_name("noop"))
        .execute(&txn)
        .unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice", "score": 10 })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(false).unwrap();
    let filter = eq_filter("_id", Bson::String("r1".into()));
    db.collection(COLLECTION)
        .find(&filter)
        .replace(doc! { "name": "Bob", "score": 50 })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let doc = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(doc.get_str("name").unwrap(), "Bob");
    assert_eq!(doc.get_i32("score").unwrap(), 50);
}

// ── Upsert trigger tests ────────────────────────────────────────

#[test]
fn upsert_fires_trigger_on_insert() {
    let db = DatabaseBuilder::new()
        .with_trigger("noop", |_: &TriggerCtx<'_>| Ok(()))
        .with_trigger("boom", |_: &TriggerCtx<'_>| {
            Err(TriggerError::Body("trigger failed!".into()))
        })
        .open(MemoryStore::new())
        .unwrap();

    let txn = db.begin(false).unwrap();
    db.collections().create(COLLECTION).execute(&txn).unwrap();
    db.collection(COLLECTION)
        .triggers()
        .create("audit", TriggerFunction::from_name("noop"))
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    // Upsert a new doc (insert path)
    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .upsert_many(vec![doc! { "_id": "r1", "name": "Alice" }])
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let result = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap();
    assert!(result.is_some());
    assert_eq!(result.unwrap().get_str("name").unwrap(), "Alice");
}

#[test]
fn upsert_fires_trigger_on_update() {
    let db = DatabaseBuilder::new()
        .with_trigger("noop", |_: &TriggerCtx<'_>| Ok(()))
        .with_trigger("boom", |_: &TriggerCtx<'_>| {
            Err(TriggerError::Body("trigger failed!".into()))
        })
        .open(MemoryStore::new())
        .unwrap();

    let txn = db.begin(false).unwrap();
    db.collections().create(COLLECTION).execute(&txn).unwrap();
    db.collection(COLLECTION)
        .triggers()
        .create("audit", TriggerFunction::from_name("noop"))
        .execute(&txn)
        .unwrap();
    db.collection(COLLECTION)
        .insert_one(doc! { "_id": "r1", "name": "Alice" })
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    // Upsert existing doc (update path)
    let txn = db.begin(false).unwrap();
    db.collection(COLLECTION)
        .upsert_many(vec![doc! { "_id": "r1", "name": "Bob" }])
        .execute(&txn)
        .unwrap();
    txn.commit().unwrap();

    let txn = db.begin(true).unwrap();
    let doc = db
        .collection(COLLECTION)
        .find(rawdoc! { "_id": "r1" })
        .iter_raw(&txn)
        .unwrap()
        .next()
        .transpose()
        .unwrap()
        .unwrap();
    assert_eq!(doc.get_str("name").unwrap(), "Bob");
}
