...
fn update_writes_merged_doc() {
    ...
    let plan = Plan::Update {
        collection: mock_collection(vec![]),
        mutation: slate_mutation::parse_mutation(&rawdoc! { "score": 100 }, "_id").unwrap(),
        source: Node::Values(docs),
    };
    let exec = Executor::new(&txn, None);
    let iter = exec.execute(plan).unwrap();
...
fn update_unchanged_skips_write() {
    ...
    let plan = Plan::Update {
        collection: mock_collection(vec![]),
        mutation: slate_mutation::parse_mutation(&rawdoc! { "status": "active" }, "_id").unwrap(),
        source: Node::Values(docs),
    };
    let exec = Executor::new(&txn, None);
    let iter = exec.execute(plan).unwrap();
...
fn replace_writes_replacement() {
    ...
    assert!(written.get_bool("replaced").unwrap());
}
...
fn full_update_pipeline() {
    ...
    let plan = Plan::Update {
        collection: mock_collection(vec!["status".into()]),
        mutation: slate_mutation::parse_mutation(&rawdoc! { "status": "archived" }, "_id").unwrap(),
        source: Node::Values(docs),
    };
    let exec = Executor::new(&txn, None);
    let iter = exec.execute(plan).unwrap();
