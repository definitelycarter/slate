...
pub fn verify_integrity(client: &mut Client, user: usize) {
    ...
    let results = client.query(&query).expect("query failed");
...
pub fn query_benchmarks(client: &mut Client) -> Vec<BenchResult> {
    let mut results = Vec::new();
    ...
    results.push(bench("tcp query: no filter (full scan)", || {
        ...
        let r = client.query(&query).expect("query failed");
        r.len()
    }));
...
pub fn concurrency_tests(addr: &str) -> Vec<BenchResult> {
    let mut results = Vec::new();
        ...
        for _ in 0..4 {
            ...
            handles.push(thread::spawn(move || {
                ...
                for _ in 0..5 {
                    ...
                    let _ = client.query(&query).expect("reader query failed");
                }
            }));
        }
