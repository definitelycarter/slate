...
pub fn generate_cells(user: usize, seq: usize) -> Vec<CellWrite> {
...
pub fn generate_batch(user: usize, start: usize, count: usize) -> Vec<(String, Vec<CellWrite>)> {
    (start..start + count)
        .map(|seq| (generate_record_id(user, seq), generate_cells(user, seq)))
        .collect()
}
